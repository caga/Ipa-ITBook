...
<%@ page import="com.nealford.art.emotherearth.entity.Product" %>
...
<jsp:useBean id="product" scope="session"
             class="com.nealford.art.emotherearth.entity.Product" />
<body>
<h1>
<%
...
<TD align='right'>
    <jsp:getProperty name="p" property="priceAsCurrency"/>
</td>
<TD><form action="showcart" method="post">
    Qty: <input type="text" size="3" name="quantity">
    <input type="hidden" name="id" value='<%= p.getId() %>'>
    <input type="submit" value="Add to cart"></form></TR>
<%
    }
%>
</TABLE>
...
<c:forEach var="page"
        items="${pagelist}">
  <c:out value="${page}" escapeXml="false"/><br>
</c:forEach>
<%--
    String[] pageList = (String[]) request.getAttribute("pageList");
    if (pageList != null) {
